...
```{r}
t <-  melt(t(grad_VL$meta.prop))
boxplot(data=t, value~Var1)
```

```{r}
t <-  melt(bench_VL$mixture)
boxplot(data=t, value~Var2)
```
...
```{r}
t <-  melt(t(true$orig.celltype))
boxplot(data=t, value~Var1)
```
```{r}
t <-  melt(t(true$merge1))
boxplot(data=t, value~Var1)
```

```{r}
t <-  melt(t(true$merge2))
boxplot(data=t, value~Var1)
```
